...
def similarity(query_words: Sequence[str], candidate: str) -> float:
    ...
    одно, и среднее утопило бы верного кандидата. Надбавка поднимает того, у кого
    совпало не одно слово: «Иванову Сергею» → «Иванов Сергей» (0.98) против «Иванов
    Пётр» (0.85). Поверх точного совпадения её не видно — оценка упирается в потолок
    1.0, и «Санта на Ленина 15а» не обходит «Санта на Ленина» на запросе «Ленина 15а»;
    различать их по номеру — работа сервера, там номер лежит отдельным полем.
    """
