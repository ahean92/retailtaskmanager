...
PHOTO_WORDS = ("фото", "сфотограф", "снимок", "снимк")
...
def resolve_photo(value: Any, text: str) -> Optional[bool]:
    """Фото — только когда о нём сказано, и первым спрашиваем ТЕКСТ, а не модель.

    Раньше первой отвечала модель, а текст оставался страховкой на случай, когда поля
    в ответе нет. Но prompt требует "photo" всегда, модель его всегда и присылает, —
    поэтому страховка не включалась ни разу, а решение целиком висело на трёхмиллиардной
    модели. В журнале это выглядело так: «обязательно с фото» срабатывало, а
    «обязательно прислать фото и координаты» — нет, без всякой закономерности.

    Прямо сказанное слово надёжнее догадки модели, поэтому порядок обратный: сначала
    отрицания («без фото»), затем упоминание фото в тексте, и только потом ответ модели —
    она ещё нужна там, где фото просят иначе, чем перечисленными словами."""
    low = normalize(text)
    if any(word in low for word in NO_PHOTO_WORDS):
        return None
    if any(word in low for word in PHOTO_WORDS):
        return True
    return _bool(value) or None
