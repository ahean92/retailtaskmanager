...
/// В неё попадает только то, за чем человек ЛИШЬ наблюдает: подписка на свою, авторскую
/// или пуловую задачу группу не меняет (#37136) — такую из пула всё ещё можно взять, а
/// «Наблюдаю» обещает «только читать».
///
/// Приёмка (#37158) добавляет три. «Ждут моей приёмки» — сразу за «Моими»: решение по
/// сданной работе — такое же дело на сейчас, как своя задача. «На приёмке» — сданное
/// мной: работа сделана, решает принимающий, поэтому только чтение и не в «Моих» —
/// плитки главной сданную не считают, и группа с ними не разойдётся (#36751). «На
/// доработке» — то, что я принимаю и вернул (или вернул коллега по подразделению):
/// сервер отдаёт такую задачу принимающему до повторной сдачи, и она не пропадает из
/// списка без объяснения, а ждёт внизу, пока не вернётся в «Ждут моей приёмки».
enum TaskGroup {
  mine('Мои'),
  awaiting('Ждут моей приёмки'),
  free('Свободные'),
...
class TaskView {
  ...
  /// Я только наблюдатель (#37135). См. Task.watchedOnly — но с наложенной очередью
  /// подписок (#37136): отписка из подвала гасит признак сразу, не дожидаясь сервера.
  final bool watchedOnly;

  /// Наблюдаю — лично или в составе подразделения (#37136), с наложенной очередью. Им
  /// рисуется пометка «наблюдаю» на строке списка у задач других групп: подписка на свою
  /// или авторскую задачу группу не меняет, и без пометки «Следить» было бы не видно.
  final bool watched;

  /// Подписан лично, с наложенной очередью (#37136): только такую подписку человек
  /// снимает с телефона — по нему рисуется «Не следить».
  final bool following;

  /// Подписка или отписка лежит в очереди и сервером ещё не подтверждена.
  final bool watchPending;

  /// Приёмка (#37158) с наложенными очередями телефона. [onAcceptance] — задача сдана:
  /// сервером (статус «На приёмке») или здесь, пока сдача в очереди (завершение или
  /// «Выполнено» у задачи с приёмкой). Сданная — только для чтения: работа сделана, и до
  /// решения делать по ней нечего.
  final bool onAcceptance;

  /// Ждёт моего решения — серверный признак, погашенный решением из очереди: принятое
  /// или возвращённое офлайн уходит из «Ждут моей приёмки» сразу.
  final bool awaitingDecision;

  /// Я только принимаю (см. Task.reviewingOnly).
  final bool reviewingOnly;

  /// Возвращена на доработку — сервером или моим возвратом из очереди — и почему.
  /// Сданная снова (в том числе здесь, офлайн) возвращённой уже не считается.
  final bool returned;
  final String? returnReason;

  /// Решение по задаче лежит в очереди и сервером ещё не подтверждено.
  ...
  /// Задача приехала ради чтения, а не работы: автор (#36844), наблюдатель (#37135),
  /// принимающий (#37158) — или работа по ней сдана и ждёт решения. Экран задачи гасит
  /// по нему бланк и взятие — причина разная, запрет один, и сервер эти попытки и так
  /// отвергает. Статус — отдельно, см. [statusChoices]: автору сервер его двигать
  /// разрешает там, где это разрешают правила переходов.
  bool get readOnly =>
      authoredOnly || watchedOnly || reviewingOnly || onAcceptance;

  /// Статусы для переключателя на карточке — из справочника [all], в его порядке.
  ///
  /// Сервер прислал, куда задачу можно перевести ([Task.nextStatusIds]), — ровно эти,
  /// плюс два текущих: серверный (выбрать его — отменить смену, ещё лежащую в очереди)
  /// и действующий с очередью (он выбран). Запрещённый правилами переход телефон
  /// больше не предлагает: раньше он уезжал в очередь и оседал в «Не отправлено».
  /// Автору и наблюдателю — тем же словом сервера: автору он разрешает то, что
  /// разрешают правила, наблюдателю — ничего.
  ///
  /// Сервер ключа не прислал (старая выдача, строка, рождённая на телефоне) — как было
  /// до правил переходов: исполнителю весь справочник, читающему ничего.
  ///
  /// Задача ждёт моего решения (#37158) — закрытие, кроме отмены, отсюда не
  /// предлагается: «Выполнено» принимающего сервер считает приёмкой, но смена статуса
  /// шла бы мимо очереди решений и мимо «кто успел раньше». Принимают кнопкой.
  /// Исполнителю задачи с приёмкой «Выполнено» подписано «Сдать на приёмку», и чип «На
  /// приёмке» рядом был бы вторым выключателем того же самого — он не предлагается.
  List<TaskStatus> statusChoices(List<TaskStatus> all) {
    final next = task.nextStatusIds;
    if (next == null) return readOnly ? const [] : all;
    bool submits(TaskStatus s) => s.closed && s.id != canceledStatusId;
    final submitChip = task.needsAcceptance == true &&
        all.any((s) => submits(s) && next.contains(s.id));
    bool offered(TaskStatus s) =>
        next.contains(s.id) &&
        !(awaitingDecision && submits(s)) &&
        !(submitChip && s.id == Task.acceptanceStatusId);
    return [
      for (final s in all)
        if (s.id == statusId || s.id == task.statusId || offered(s)) s
    ];
  }

  /// Код статуса «Отменено» в справочнике: отмена — закрытие, но не приёмка.
  static const canceledStatusId = 'canceled';

  /// Возвращена, и последнее выполнение, по выдаче, — сданное (#37158): экран
  /// выполнения, открытый сейчас, начинает новый раунд (сервер — Execution.restartable).
  /// По выдаче, а не по кэшу экрана: кэш может не знать, что сданное завершено.
  bool get restartsExecution =>
      returned &&
      task.executions.isNotEmpty &&
      task.executions.last.finished;

  /// «Следить» имеет смысл (#37136): не наблюдаю вовсе, задача не моя и не закрыта. В
  /// «Моих» человек сам исполнитель — всё, что случается с задачей, и так приходит ему, и
  /// кнопка лишь спорила бы с рабочими. Наблюдающий в составе подразделения лично не
  /// подписывается: уведомления у него уже есть, и личная подписка ничего бы не прибавила.
  /// Закрытой (в том числе завершённой на телефоне) уведомлений больше не будет.
  /// Сданная мной и ждущая моего решения (#37158) — тоже без кнопки: исход приёмки
  /// приходит обоим и так, это их собственное дело.
  bool get canFollow =>
      !watched &&
      !closed &&
      group != TaskGroup.mine &&
      group != TaskGroup.submitted &&
      group != TaskGroup.awaiting;
...
enum TaskFilter {
  ...
  static TaskFilter parse(String? code) => switch (code) {
        ...
        'acceptance' => TaskFilter.acceptance,
        _ => TaskFilter.all,
      };

  /// Четыре фильтра из пяти — двойники плиток главной, и тап по плитке открывает
  ...
  /// открывает (#36944, продолжение #36751). «Ждут приёмки» (#37158) — плитка
  /// myAcceptance от canDecide, тот же предикат, что awaitingDecision в выдаче, и
  /// фильтр — ровно группа «Ждут моей приёмки». «Все задачи» — единственный чип без
  /// плитки-двойника: он показывает список целиком, всеми группами, и через него
  /// видно то, что остальные прячут.
  bool matches(TaskView v) => switch (this) {
        ...
        TaskFilter.acceptance => v.group == TaskGroup.awaiting,
      };
}
