...
class TaskView {
  ...
  /// Экран задачи гасит по нему бланк и взятие — причина разная, запрет один, и сервер
  /// обе эти попытки и так отвергает. Статус — отдельно, см. [statusChoices]: автору
  /// сервер его двигать разрешает там, где это разрешают правила переходов.
  ...
  List<TaskStatus> statusChoices(List<TaskStatus> all) {
    ...
    return [
      for (final s in all)
        if (s.id == statusId || s.id == task.statusId || next.contains(s.id)) s
    ];
  }
  ...
  bool get canFollow => !watched && !closed && group != TaskGroup.mine;
