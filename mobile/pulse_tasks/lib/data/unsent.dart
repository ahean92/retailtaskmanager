...
class UnsentKind {
  ...
  static const decision = 'decision'; // принять/вернуть (decision_outbox, #37158)
...
/// Та же причина для строки со своим зачином — «Не принято: …», «Не удалось
/// завершить: …»: отказ сервера — его словами, без «Отклонено сервером», обрыв
/// связи — как в [syncFailureText], а не строкой исполняющей среды.
String failureReason(Object e) =>
    e is ApiException ? e.message : syncFailureText(e);
...
Future<List<UnsentOp>> loadUnsentOps(LocalDb db) async {
  ...
  // решения по сданным задачам (#37158)
  for (final r in await db.tasks.getDecisionOutbox()) {
    final id = r['taskId'] as String;
    ops.add(UnsentOp(
      kind: UnsentKind.decision,
      taskId: id,
      title: titleOf(id),
      detail: r['action'] == 'accept'
          ? 'Принять результат'
